...
Hierfür werden zunächst alle Trainingsbilder mit Hilfe eines Octree quantisiert (Abbildung \ref{fig:octree}). Der Farbraum wird zunächst in acht Unterräume unterteilt, was durch eine gleichmäßige Unterteilung jedes Farbkanals in zwei Bereiche erreicht wird. Diese Unterräume werden rekursiv wieder in acht Teile zerlegt, bis die Anzahl an Pixeln, die durch diesen Bereich repräsentiert werden unterhalb eines Grenzwert liegt. 
\begin{figure}
\center
...
\caption[Beispiel Histogramspezifikation]{Vorgang bei der Histogrammspezifikation. Die CDF(Cumulated Density Function) eines Bildes $\mathbf{x1}$ soll auf die CDF von Bild $\mathbf{x2}$ abgebildet werden. Die Pfeile verbildlichen dabei das genaue Vorgehen\cite{url_hist_spe}.\label{fig:equalisierung}}
\end{figure}
