...
Das menschliche Gehirn ist innerhalb gewisser Grenzen in der Lage die Farbe eines Objektes unabhängig von dessen Beleuchtung zu bestimmen. Diese Fähigkeit ist die Grundlage für eine Gruppe von Ansätzen, die als Farbkonstanzalgorithmen bezeichnet werden \citep[S. 18]{rangayyan2011color}.
...
Die Schwächen des Verfahrens, zeigen sich vor allem, wenn sich Eingangs- und Referenzbild zu sehr unterscheiden. In diesem Fall kommt es zu Artefakten, bei denen monochrome Flächen nach der Anpassung starke Unterschiede aufweisen. Ein Beispiel hierfür ist in Abbildung \ref{fig:bsp_ranking} gezeigt.
...
\caption[Beispiel Auswirkung Farbranking]{Die Abbildung verdeutlicht die Schwächen des Farbrankingverfahrens. \textbf{(a)} zeigt das Originalbild, \textbf{(b)} die Referenz. Im Ergebnisbild \textbf{(c)} zeigen sich deutliche Artefakte im Hintergrund. Außerdem ist der Erythrozyt oben im Bild nach der Anpassung stark gefärbt.\label{fig:bsp_ranking}}
\end{figure}

\subsection[Anpassung nach Reinhard]{Anpassung nach Reinhard}\label{sec:reinhard}
Ein andere Methode, bei der die Akquisition keine Rolle spielt, wird von \citeeig{reinhard2001color} vorgeschlagen. Ziel ist es, die Anpassung in möglichst schwach korrelierenden Kanälen durchzuführen. Ein Farbraum, der diese Eigenschaft für natürliche Bilder erfüllt ist der von \citeeig{ruderman1998statistics} vorgeschlagene $l\alpha\beta$-Raum. Die Umrechnung erfolgt dabei in mehreren Schritten. 
\begin{itemize}
...
\caption[Color Deconvolution nach Macenko]{\textbf{(a)} originales Bild, \textbf{(b)} dritter Kanal, der aufgrund der sehr guten Aufteilung der Informationen über die Grundfarbstoffe in \textbf{(c)} und \textbf{(d)} leer erscheint.\label{fig:cd_macenko} }
\end{figure}

\citeauthor{macenko2009method} liefern auch einen Ansatz für die Normalisierung. Hierbei wird für jeden vorkommenden Farbstoff ein Histogramm berechnet, welches jedoch nur jene Pixel berücksichtigt, die überwiegend den entsprechenden Farbstoff beinhalten.
Die Histogramme werden anschließend so skaliert, dass alle über das gleiche Pseudomaximum verfügen, um sie miteinander vergleichbar zu machen (siehe Formel \ref{equ:scale}). Dieses Vorgehen fällt in den Bereich der Histogrammanpassung und hat den Effekt, dass das Histogramm entweder gedehnt oder gestaucht wird. Welche der beiden Möglichkeiten eintritt hängt von der Größe des Skalierungsfaktors $k$ ab. Ist $k < 1$, entspricht dies einer Stauchung, ist $k > 1$ einer Dehnung. $k$ ist dabei abhängig vom realen Maximum $m$ und dem Zielmaximum $m_{ref}$. In Formel \ref{equ:scale} repräsentiert $x_i$ einen Wert aus dem ursprünglichen Histogramm, $\hat x_i$ ist der entsprechende angepasste Wert.

\begin{equation}\label{equ:scale}
\hat x_i = \frac{m_{ref}}{m}x_i = kx_i
\end{equation}

Bei diesem Verfahren wird vor allem berücksichtigt, dass Proben gebleicht worden sein könnten, dieser Effekt wird hierdurch reduziert. Allerdings wird nicht bedacht, dass sich die Proben auch inhaltlich stark unterscheiden könnten. Das Ergebnis der Anpassung zweier Bilder ist in Abbildung \ref{fig:macenko_result} gezeigt.
...
Hierfür werden zunächst alle Trainingsbilder mit Hilfe eines Octree quantisiert \cite{gervautz1988simple}. Der Farbraum wird dafür in acht Unterräume unterteilt, was durch eine gleichmäßige Aufteilung der drei Farbkanäle in zwei Bereiche, es ergeben sich $2^3 = 8$ Kombinationen, erreicht wird. Diese Unterräume werden rekursiv wieder in acht Teile zerlegt, bis die Anzahl an Pixeln, die durch diesen Bereich repräsentiert werden unterhalb eines Grenzwertes liegt. In Abbildung \ref{fig:octree} ist der  Aufbau eines Octree dargestellt.
\begin{figure}
\center
...
\item{Zuordnung zu einer Klasse $\theta(c) \in \lbrace \text{GEFÄRBT, HINTERGRUND, SONSTIGES}\rbrace$ mit Hilfe der folgenden Regel \ref{equ:thres_rule}:
