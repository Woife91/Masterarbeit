...
\subsection{Farbkonstanzalgorithmen}\label{sec:farbkonstanz}
...
\quad
\caption[Beispiel einer Histogrammequalisierung]{ \label{fig:EQGraubild}}
\end{figure}
...
Die Logik hinter diesem Vorgehen ist, dass durch das Wählen von Minimum und Maximum, jeder Wert dazwischen als lineare Summe dieser beiden Vektoren ausdrückbar ist.

\citeauthor{macenko2009method} liefern auch einen Ansatz für die Normalisierung. Hierbei wird für jeden vorkommenden Farbstoff ein Histogram berechnet, welches jedoch nur jene Pixel berücksichtigt, die überwiegend diesen Farbstoff beinhalten.
Die Histogramme werden anschließend so skaliert, dass alle das gleiche Pseudomaximum haben um sie miteinander vergleichbar zu machen. Hierbei wird vor allem berücksichtigt, dass Proben gebleicht worden sein könnten und dieser Effekt reduziert. Allerdings wird nicht berücksichtigt, dass sich die Proben auch inhaltlich stark unterscheiden könnten.
