...
Die Anpassungsvariante mittels B-Spline und den manuell von \citeauthor{landini2016} bestimmten Giemsa Stainvektoren soll nun den übrigen getesteten Methoden und den Originalbildern gegenübergestellt werden. Im Einzelnen wurden hierfür jeweils die besten getesteten Parameterkombination für B-Spline mit durch die Methode von Macenko bestimmten Vektoren, für die Skalierung auf ein festes Maximum nach Color Deconvolution (CD), für die Anpassung von Mittelwert und Standardabweichung nach CD und die Anpassung nach Reinhard. Anhand eines Sets von 25 Originalbildern sind in Anhang \ref{app:norm} die Veränderungen des visuellen Eindrucks gezeigt. 
...
Für die Klassifikation wurden zehn verschiedene Experimente durchgeführt, welche in Tabelle \ref{tab:ex_zuordnung} erläutert sind. Der Ansatz nach Reinhard wurde nicht weiter verfolgt, da die Farbinformationen bei diesem Ansatz extrem verfälscht werden. Die Experimente 1-4 verwenden B-Splines nach Color Deconvolution und Stainvektoren nach Landini. Unterschieden wird dabei nach verwendeter Referenz. Experiment 1 repräsentiert dabei das in Kombination von ganzer Zelle und Zellkern beste Segmentierungsresultat. In Experiment 5 wurden unveränderte Bilder mit automatischer Segmentierung getestet. Die Experimente 6-8 beinhalten die restlichen implementierten Anpassungsmethoden B-Spline mit Stainvektoren nach Macenko, Mittelwert und Standardabweichung und Fixes Maximum. Experiment 9 verwendet die Originalbilder und die manuelle Handsegmentierung, Experiment 10 ebenfalls die Originalbilder jedoch mit der automatischen Segmentierung der Daten aus Experiment 1.
...
In Abbildung \ref{fig:acc_prec} sind die Trefferquoten und Genauigkeiten für die verschiedenen Versuche dargestellt. Deutlich die schwächsten Ergebnisse weist Experiment 5 auf, während die Klassifizierung unter Verwendung der Handsegmentierung (Experiment 9) die besten Ergebnisse erzielte. Mit vorheriger Farbnormalisierung ist Experiment 1, welches die beste automatische Segmentierung beinhaltet, am erfolgreichsten. Bei Versuch 10, bei welchem nur die bessere automatische Segmentierung aus Versuch 1 übernommen wurde, ist sowohl Trefferquote als auch Genauigkeit deutlich besser als für Experiment 5, während sich die Resultate gegenüber Versuch 1 nur wenig unterscheiden. Bei Nummer 1 liegt die Trefferquote bei 0.44 bzw. 0.58 unter Berücksichtigung der Toleranzfelder, Versuch ist mit 0.40 bzw. 0.56 hier schwächer. Dafür ist die Genauigkeit mit 0.39 bzw. 0.55 gegenüber 0.39 bzw. 0.54 minimal verbessert.   
...
Die Tabellen \ref{tab:ex01}-\ref{tab:ex10} zeigen die Vertauschungsmatrizen für die wichtigsten Experimente. In grün sind dabei korrekt klassifizierte Zellen aufgeführt, gelb sind tolerierbare Fehler. Wie schon bei der Segmentierung gibt es bei der Klassifikation Unterschiede zwischen den einzelnen Zellarten. Zum einen variiert die Trefferquote im gleichen Experiment, zum anderen gibt es Zellarten, welche in den verschiedenen Versuchen deutlich unterschiedliche Resultate liefern. Ein Beispiel hierfür sind eosinophile Zellen, wo in Versuch 5 nur eine von sechs als solche erkannt wurde, während in Versuch 9 alle Vertreter als solche identifiziert wurden, jedoch drei andere fälschlicherweise ebenfalls als Eosinophile deklariert wurden.  
